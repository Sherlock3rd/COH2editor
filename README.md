...
3. 运行 `scripts/start-worldbuilder.ps1` 启动 World Builder。
4. 将自主制作地图的工作副本放在 `workspace/maps/`；外部参考资料放在 `workspace/references/`。
5. 让助手生成的报告、预览和导出文件分别进入 `artifacts/reports/`、`artifacts/previews/` 和 `artifacts/exports/`。

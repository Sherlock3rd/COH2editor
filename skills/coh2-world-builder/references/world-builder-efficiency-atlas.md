...
- 接手历史场景后，先导出高度图并记录 min/max 与分位数。若计划是缓坡小镇却出现孤立的极深坑、尖峰或远超设计区间的 extrema，禁止逐点修补；备份后用 `Set Value + Apply Globally` 建立安全基准，再分批重建台地和缓坡。
- 高度验收采用“数值 + 三维相机”双证据：数值排除隐藏尖峰/深坑，出生、镇区、侧翼三处游戏相机排除遮挡、断坡和视觉上仍然平坦的问题。
- 旧道路不合理时先检查 Tile Layers，而不是继续叠画。顶层纹理可能只是复用了历史 alpha 蒙版；重新 Assign 会让旧三角形/环形路网再次出现。
